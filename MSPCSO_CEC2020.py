...
def RunMSPCSO(func):
    ...
    np.savetxt("./MSPCSO_Data/CEC2020/F" + str(FuncNum+1) + "_" + str(DimSize) + "D.csv", All_Trial_Best, delimiter=",")
...
def main(dim):
    ...
    for i in range(len(CEC2020)):
        FuncNum = i
        RunMSPCSO(CEC2020[i].evaluate)


if __name__ == "__main__":
    if os.path.exists('./MSPCSO_Data/CEC2020') == False:
        os.makedirs('./MSPCSO_Data/CEC2020')
    Dims = [10, 20]
    for Dim in Dims:
        main(Dim)
